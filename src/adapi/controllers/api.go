...
func (u *ApiController) Get() {
	...
	lib.ALogger.Info("Request form %v : %v:", remoteIp, string(reqJson))
	...
	lib.ALogger.Info("Response to %v : %v ", remoteIp, string(responseJson))
	if callback != "" {
...
func getAdData(adReuest AdRequest, countryCode string, class_slice []int) lib.Response {
	...
	if adReuest.JsType == 2 || adReuest.JsType == 0 {
		if jsAdOut.Id == 0 {
			getDeployAdList(adReuest, countryCode, class_slice)
		} else {
